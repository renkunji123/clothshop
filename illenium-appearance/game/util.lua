...
--- HGRP: component 9 (body armor mesh) is owned by hgrp_clothing_capture slot 108 only.
local function zeroBodyArmorInComponents(components)
...
local function clearBodyArmorComponent(ped)
    if ped and ped ~= 0 and DoesEntityExist(ped) then
        SetPedComponentVariation(ped, BODY_ARMOR_COMPONENT_ID, 0, 0, 0)
    end
end
...
local function getPedComponents(ped)
    ...
    for i = 1, size do
        ...
        if componentId == BODY_ARMOR_COMPONENT_ID then
            drawable = 0
            texture = 0
        end
        components[i] = {
...
local function setPedComponent(ped, component)
    if component then
        if component.component_id == BODY_ARMOR_COMPONENT_ID then
            return
        end
...
local function setPedAppearance(ped, appearance)
    if appearance then
        zeroBodyArmorInComponents(appearance.components)
        setPedComponents(ped, appearance.components)
        clearBodyArmorComponent(ped)
