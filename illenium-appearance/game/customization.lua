...
local creationHiddenHairCollections

local function getCreationHiddenHairCollections()
    if creationHiddenHairCollections then
        return creationHiddenHairCollections
    end

    creationHiddenHairCollections = {}
    local cfg = Config.HidePackHairOnCharacterCreation
    if not cfg or not cfg.enabled or type(cfg.collections) ~= 'table' then
        return creationHiddenHairCollections
    end

    for i = 1, #cfg.collections do
        local name = cfg.collections[i]
        if type(name) == 'string' and name ~= '' then
            creationHiddenHairCollections[string.lower(name)] = true
        end
    end
...
local function isCharacterCreationSession()
    return config and config.enableExit == false
end

local function appendCreationPackHairBlacklist(ped, blacklistSettings)
    if not isCharacterCreationSession() then return end

    local hiddenCollections = getCreationHiddenHairCollections()
    if not next(hiddenCollections) then return end

    if not GetPedCollectionNameFromDrawable then return end

    local maxDrawable = GetNumberOfPedDrawableVariations(ped, 2) - 1
    if maxDrawable < 0 then return end

    local existing = {}
    for i = 1, #blacklistSettings.drawables do
        existing[blacklistSettings.drawables[i]] = true
    end

    for drawable = 0, maxDrawable do
        if not existing[drawable] then
            local ok, collection = pcall(GetPedCollectionNameFromDrawable, ped, 2, drawable)
            if ok and collection and hiddenCollections[string.lower(collection)] then
                blacklistSettings.drawables[#blacklistSettings.drawables + 1] = drawable
                existing[drawable] = true
            end
        end
    end
end
...
local function getHairSettings(ped)
    ...
    if client.isPedFreemodeModel(ped) then
        ...
        appendCreationPackHairBlacklist(ped, blacklistSettings)
    end
...
function client.startPlayerCustomization(cb, conf)
    ...
    SendNuiMessage(json.encode({
        ...
        payload = {
            ...
            hgrpCatalog = config and config.hgrpCatalog == true or nil,
        }
    }))
end
...
exports("getComponentSettings", function(ped, componentId)
    return client.getComponentSettings(ped, componentId)
end)

exports("getPropSettings", function(ped, propId)
    return client.getPropSettings(ped, propId)
end)
