...
-- HGRP: event sớm hơn QBCore OnPlayerLoaded (tránh flash ped mặc định Michael/đeo dù)
AddEventHandler('playerSpawned', function()
    if didInitFade then return end
    didInitFade = true
...
function InitAppearance()
    ...
    -- HGRP: đảm bảo vẫn đang hide nếu InitAppearance chạy sau playerSpawned
    if not IsScreenFadedOut() then
        DoScreenFadeOut(0)
    end
    ...
    lib.callback("illenium-appearance:server:getAppearance", false, function(appearance)
        if not appearance then
            ...
            TriggerEvent('illenium-appearance:client:appearanceApplied', false)
            return
        end

        client.setPlayerAppearance(appearance)
        -- HGRP: không gỡ đồ ở đây — hgrp_clothing_capture apply sau khi inventory + skin sẵn sàng.
        -- (Gọi unequipClothing khi slot 104+ trống nhưng túi đã sync → cởi trần lúc relog.)
        if Config.PersistUniforms then
            LoadPlayerUniform()
        end
        -- HGRP: hgrp_clothing_capture sync sau appearanceApplied + setPlayerInventory.
        showPlayerPed()
        if IsScreenFadedOut() then DoScreenFadeIn(250) end
        TriggerEvent('illenium-appearance:client:appearanceApplied', true)
    end)
    ResetBlips()
    if Config.BossManagedOutfits then
        Management.AddItems()
...
-- HGRP: exports cho UI salon tóc custom
exports('getPedAppearance', function(ped)
    return client.getPedAppearance(ped)
end)

exports('setPedHair', function(ped, hair, tattoos)
    return client.setPedHair(ped, hair, tattoos)
end)

exports('setPedHeadOverlays', function(ped, headOverlays)
    return client.setPedHeadOverlays(ped, headOverlays)
end)
