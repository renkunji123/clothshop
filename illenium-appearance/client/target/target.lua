...
local function SetupStoreTargets()
    for k, v in pairs(Config.Stores) do
        ...
        if v.type == "barber" then
            action = function()
                TriggerEvent('hgrp_barbershop:client:open', k)
            end
        elseif v.type == "clothing" then
            if type(v.hgrpShopKey) ~= 'string' or v.hgrpShopKey == '' then
                goto continue_store_target
            end
            action = function()
                TriggerEvent('hgrp_clothingshop:client:open', k)
            end
