...
local BODY_ARMOR_COMPONENT_ID = 9

--- HGRP: never load/save body armor mesh from character skin — slot 108 owns comp 9.
local function stripBodyArmorFromAppearance(appearance)
    if not appearance or type(appearance.components) ~= 'table' then
        return appearance
    end

    for i = 1, #appearance.components do
        local component = appearance.components[i]
        if component and component.component_id == BODY_ARMOR_COMPONENT_ID then
...
lib.callback.register("illenium-appearance:server:getAppearance", function(source, model)
    local citizenID = Framework.GetPlayerID(source)
    return stripBodyArmorFromAppearance(Framework.GetAppearance(citizenID, model))
end)
...
RegisterServerEvent("illenium-appearance:server:saveAppearance", function(appearance)
    ...
    if appearance ~= nil then
        Framework.SaveAppearance(stripBodyArmorFromAppearance(appearance), citizenID)
    end
end)
...
lib.callback.register("illenium-appearance:server:tryChargeCustomer", function(source, shopType)
    local money = getMoneyForShop(shopType)
    if Framework.RemoveMoney(source, "cash", money) then
        lib.notify(source, {
            title = _L("purchase.store.success.title"),
            description = string.format(_L("purchase.store.success.description"), money, shopType),
            type = "success",
